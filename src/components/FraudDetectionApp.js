...
const FraudDetectionApp = () => {
  ...
  const calculateFraudProbability = () => {
    ...
    setTimeout(() => {
      // Simple heuristic model based on the features from the notebook
      let score = 0;
      const factors = [];
      
      // Amount factor (higher amounts increase risk)
      if (formData.amount > 1000) {
        ...
        factors.push({
          ...
          impact: 'high',
          description: 'Transactions over $1,000 have higher fraud rates'
        });
        ...
        factors.push({
          ...
          value: formData.amount,
          impact: 'medium',
          description: 'Transactions over $500 have elevated fraud rates'
        });
      } else if (formData.amount > 200) {
        score += 0.15;
        factors.push({
          name: 'Moderate transaction amount',
          value: formData.amount,
          impact: 'low',
          description: 'Transactions between $200-$500 have slightly higher fraud risk'
        });
      } else if (formData.amount < 10) {
        score += 0.1;
        factors.push({
          name: 'Very small transaction amount',
          value: formData.amount,
          impact: 'low',
          description: 'Very small amounts can indicate testing before larger fraud attempts'
        });
      }
      
      // Hour factor (late night has higher risk)
      if (formData.hour >= 0 && formData.hour <= 5) {
        score += 0.25;
        factors.push({
          name: 'Late night transaction',
          value: `${formData.hour}:00`,
          impact: 'high',
          description: 'Transactions between midnight and 5AM show higher fraud rates'
        });
      } else if (formData.hour >= 22) {
        score += 0.15;
        factors.push({
          name: 'Evening transaction',
          value: `${formData.hour}:00`,
          impact: 'medium',
          description: 'Transactions after 10PM have elevated fraud risk'
        });
      }
      
      // Distance factor (larger distances increase risk)
      if (formData.distance > 100) {
        score += 0.3;
        factors.push({
          name: 'Large customer-merchant distance',
          value: `${formData.distance} km`,
          impact: 'high',
          description: 'Transactions where customer and merchant are far apart have higher fraud risk'
        });
      } else if (formData.distance > 50) {
        score += 0.1;
        factors.push({
          name: 'Moderate customer-merchant distance',
          value: `${formData.distance} km`,
          impact: 'medium',
          description: 'Distance between customer and merchant is somewhat unusual'
        });
      }
      
      // Day of week factor (weekends slightly higher risk)
      if (formData.dayOfWeek >= 5) {
        score += 0.05;
        factors.push({
          name: 'Weekend transaction',
          value: formData.dayOfWeek === 5 ? 'Saturday' : 'Sunday',
          impact: 'low',
          description: 'Weekend transactions have slightly higher fraud rates'
        });
      }
      
      // City population (smaller cities might have less fraud protection)
      if (formData.cityPop < 50000) {
        score += 0.1;
        factors.push({
          name: 'Small city population',
          value: formData.cityPop.toLocaleString(),
          impact: 'low',
          description: 'Transactions in smaller cities may have less fraud monitoring'
        });
      }
      
      // Apply some randomness for demo purposes
      score = Math.min(0.95, Math.max(0.01, score + (Math.random() * 0.1 - 0.05)));
      
      setRiskScore(score);
      setRiskFactors(factors);
      setPrediction(score > 0.5 ? "Fraudulent" : "Legitimate");
      setLoading(false);
    }, 1500);
  };
  ...
  // Prepare feature importance data for comparison chart
  const featureImportance = [
    { name: 'Amount', value: 0.28 },
    { name: 'Distance', value: 0.24 },
    { name: 'Hour', value: 0.14 },
    { name: 'City Pop', value: 0.13 },
    { name: 'Month', value: 0.08 },
    { name: 'Day', value: 0.07 },
    { name: 'Day of Week', value: 0.06 }
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 bg-gray-50 p-6 rounded-xl shadow">
          ...
          <div className="space-y-5">
            ...
            <button
              onClick={calculateFraudProbability}
              disabled={loading}
